...
!function($) {
    ...
    $.fn.validation.defaults = {
        validRules : [
					...
					{name: 'name', validate: function(value) {return (!($.trim(value).length>2 && $.trim(value).length<30));}, defaultMsg: '请输入店铺名称'},
					...
					{name: 'mobile', validate: function(value) {return (!/^[1-9]\d{10}$/.test(value));}, defaultMsg: '手机号必须是11位数字'},
        ]
    };
    ...
    var validateField = function(field, valid) { // 验证字段
        ...
        var controls = el.parents('.controls'), controlGroup = el.parents('.form-group'), errorEl = controls.children('.help-block, .help-inline');
        ...
        if (error) {
            if (!controlGroup.hasClass('has-error')) {
                if (errorEl.length > 0) {
                    ...
                    controls.append('<span class="help-block">'+errorMsg+'</span>');
                }
            ...
            if (fieldState) {
                ...
                if(el.parents('.prize_modlum')){
					var extra=true;
                }
                controlGroup.attr('class','form-group');
                if(extra){
                	controlGroup.addClass('prize_modlum');
                }
